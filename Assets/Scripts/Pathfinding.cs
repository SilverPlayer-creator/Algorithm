...
public class PAthfinding : MonoBehaviour
{
    ...
    List<Node> NeighbourNodes(Node neighbourNode)
    {
        ...
        //right side
        xCheck = neighbourNode.GridX + 1;
        yCheck = neighbourNode.GridY;
        ...
        //top side
        xCheck = neighbourNode.GridX;
        yCheck = neighbourNode.GridY + 1;
        if (xCheck >= 0 && xCheck < gridSizeX)
        {
            if (yCheck >= 0 && yCheck < gridSizeY)
            {
        ...
        if (drawGizmos)
        {
            ...
            foreach (Node node in grid)
            {
                ...
                if (showGCost)
                {
                    ...
                    Handles.Label((Vector2)node.Position + (new Vector2(-.5f, .5f) * nodeRadius), "G: " + node.GCost.ToString(), style);
                }
                if (showHCost)
                {
                    style.normal.textColor = Color.blue;
                    Handles.Label((Vector2)node.Position + (new Vector2(.3f, .5f) * nodeRadius), "H: " + node.HCost, style);
                }
                if (showFCost)
                {
